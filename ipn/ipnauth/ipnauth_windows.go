...
func GetConnIdentity(logf logger.Logf, c net.Conn) (ci *ConnIdentity, err error) {
	...
	cwt, ok := c.(*safesocket.ConnWithToken)
	if !ok {
		return nil, fmt.Errorf("not a ConnWithToken: %T", c)
	}
	ci.pid, err = cwt.ClientPID()
	if err != nil {
		return nil, err
	}
	return ci, nil
...
func (ci *ConnIdentity) WindowsToken() (WindowsToken, error) {
	var cwt *safesocket.ConnWithToken
	var ok bool
	if cwt, ok = ci.conn.(*safesocket.ConnWithToken); !ok {
		return nil, fmt.Errorf("not a ConnWithToken: %T", ci.conn)
	}
