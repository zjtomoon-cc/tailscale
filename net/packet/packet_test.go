...
func TestNatChecksumsV6UDP(t *testing.T) {
	...
	b.Encode(&header.IPv6Fields{
		...
		HopLimit:          16,
		SrcAddr:           tcpip.Address(a1.AsSlice()),
		DstAddr:           tcpip.Address(a2.AsSlice()),
	})
	...
	xsum := header.PseudoHeaderChecksum(
		...
		tcpip.Address(a1.AsSlice()),
		tcpip.Address(a2.AsSlice()),
		uint16(header.UDPMinimumSize+32),
	)
	xsum = checksum.Checksum(b.Payload()[header.UDPMinimumSize:], xsum)
	udp.SetChecksum(^udp.CalculateChecksum(xsum))
	if !udp.IsChecksumValid(tcpip.Address(a1.AsSlice()), tcpip.Address(a2.AsSlice()), checksum.Checksum(b.Payload()[header.UDPMinimumSize:], 0)) {
	...
	p.UpdateSrcAddr(a2)
	if !udp.IsChecksumValid(tcpip.Address(a2.AsSlice()), tcpip.Address(a2.AsSlice()), checksum.Checksum(b.Payload()[header.UDPMinimumSize:], 0)) {
	...
	p.UpdateDstAddr(a1)
	if !udp.IsChecksumValid(tcpip.Address(a2.AsSlice()), tcpip.Address(a1.AsSlice()), checksum.Checksum(b.Payload()[header.UDPMinimumSize:], 0)) {
...
func TestNatChecksumsV6TCP(t *testing.T) {
	...
	// xsum = checksum.Checksum(b.Payload()[header.TCPMinimumSize:], xsum)
	tcp.SetChecksum(^tcp.CalculateChecksum(xsum))

	if !tcp.IsChecksumValid(tcpip.Address(a1.AsSlice()), tcpip.Address(a2.AsSlice()), 0, 0) {
		t.Fatal("test broken; initial packet has incorrect checksum")
	}
	...
	// Update the source address of the packet to be the same as the dest.
	p.UpdateSrcAddr(a2)
	if !tcp.IsChecksumValid(tcpip.Address(a2.AsSlice()), tcpip.Address(a2.AsSlice()), 0, 0) {
		t.Fatal("incorrect checksum after updating source address")
	}

	// Update the dest address of the packet to be the original source address.
	p.UpdateDstAddr(a1)
	if !tcp.IsChecksumValid(tcpip.Address(a2.AsSlice()), tcpip.Address(a1.AsSlice()), 0, 0) {
		t.Fatal("incorrect checksum after updating destination address")
	}
}
